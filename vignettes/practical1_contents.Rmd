...
1) Hurricane Shary hit the Americas in October 2010. We've managed to attain the approximate longitude and latitude for the storm every 6 hours it was active. You can see the data in table BLAH. To get you started, I've created the matrix of longitudes and latitudes. Turn the matrix into a **sf** linestring and plot it
    ```{r, fig.keep='none', echo = TRUE}
...
2) 12 hours after Shary was declassified to a tropical storm, it was upgraded again to a hurricane. You can see this data in table BLAH. Create a matrix called `shary_matrix2`, using code similar to that of question 1. Combine the two matrices `shary_matrix` and `shary_matrix2` into a multilinestring
    ```{r}
