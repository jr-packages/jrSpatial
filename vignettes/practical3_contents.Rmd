...
```{r, popdensity, echo = FALSE, fig.keep = 'none'}
library("dplyr")
ukgeom = 
ukgeom  %>%
...
# Notice this output is recognised as a data frame, but not as being an sf data frame. This means that functions like tmap() will not work. 
```

3. Use `filter()` to find the regions with the following characteristics.
* Have a population (in 2017) greater than 1,600,000.
* Has a disposable income of less than 15000 or and unemployment rate of greater than 5.
